...
class VariantApiService {
    static function variationApi() {
        $data = [
            ...
            'utility'        => Color::where( ['status' => 'active'] )->select( 'id', 'name' )->get(),
            ...
            'warehouse'      => Warehouse::where( ['status' => 'active'] )->latest()->select( 'id', 'name' )->get(),
            'customer'       => Customer::where( 'status', 'active' )->select( 'id', 'customer_name', 'phone', 'email', 'address' )->get(),
            'resource'       => SaleOrderResource::latest()->where( 'status', 'active' )->select( 'id', 'name' )->get(),
        ];
